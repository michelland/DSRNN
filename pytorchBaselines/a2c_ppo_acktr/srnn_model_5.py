...
class HumanNodeRNN(RNNBase):
    ...
    def __init__(self, config):
        ...
        self.edge_attention_embed = nn.Linear(self.edge_rnn_size*2, int(self.embedding_size / 2))

        # Linear layer to embed attention module output (obstacles)
        self.edge_attention_embed_obstacles = nn.Linear(self.edge_rnn_size * 2, int(self.embedding_size / 2))
    ...
    def forward(self, pos, h_temporal, h_spatial_other, h_spatial_obstacles, h, masks):
        '''
        ...
        h_edges_embedded_humans = self.relu(self.edge_attention_embed(h_edges))

        # Concat both the embeddings (obstacles)
        h_edges_obstacles = torch.cat((h_temporal, h_spatial_obstacles), -1)
        h_edges_embedded_obstacles = self.relu(self.edge_attention_embed_obstacles(h_edges_obstacles))

        h_edges_embedded = torch.cat((h_edges_embedded_humans, h_edges_embedded_obstacles), -1)
...
class SRNN5(nn.Module):
    ...
    def __init__(self, obs_space_dict, config, infer=False):
        ...
        self.attn_humans = EdgeAttention(config)
        self.attn_obstacles = EdgeAttention(config)
    ...
    def forward(self, inputs, rnn_hxs, masks, infer=False):
        ...
        # Do forward pass through attention module HUMANS
        hidden_attn_weighted_humans, attention_weights_humans = self.attn_humans(output_temporal, output_spatial_humans)


        # Do forward pass through attention module OBSTACLES
        hidden_attn_weighted_obstacles, attention_weights_obstacles = self.attn_obstacles(output_temporal, output_spatial_obstacles)

        #concat attention weights humans and obstacles
        self.attention_weights = torch.cat((attention_weights_humans, attention_weights_obstacles), -2)
        ...
        outputs, h_nodes \
            = self.humanNodeRNN(nodes_current_selected, output_temporal, hidden_attn_weighted_humans, hidden_attn_weighted_obstacles, hidden_states_node_RNNs, masks)
