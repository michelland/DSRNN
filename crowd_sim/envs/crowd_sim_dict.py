...
class CrowdSimDict(CrowdSim):
    ...
    def set_robot(self, robot):
        ...
        if self.config.robot.policy == 'srnn2' or self.config.robot.policy == 'srnn' or self.config.robot.policy == 'srnn3' or self.config.robot.policy == 'srnn4' or self.config.robot.policy == 'srnn5':
            ...
            d['spatial_edges_obstacles'] = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(self.map.obstacle_num, 2), dtype=np.float32)
    ...
    def generate_ob(self, reset):
        ...
        if self.config.robot.policy == 'srnn2' or self.config.robot.policy == 'srnn' or self.config.robot.policy == 'srnn3' or self.config.robot.policy == 'srnn4' or self.config.robot.policy == 'srnn5':
            ...
            for i in range(self.human_num):
                relative_pos = np.array([self.last_human_states[i, 0] - self.robot.px, self.last_human_states[i, 1] - self.robot.py])
                ob['spatial_edges_humans'][i] = relative_pos
            ob['spatial_edges_obstacles'] = np.zeros((self.map.obstacle_num, 2))
            for j in range(self.map.obstacle_num):
                relative_pos = np.array([self.map.obstacles_rectangle[j].px - self.robot.px, self.map.obstacles_rectangle[j].py - self.robot.py])
                ob['spatial_edges_obstacles'][j] = relative_pos
    ...
    def reset(self, phase='train', test_case=None):
        ...
        if self.map_random:
            self.map.generate_random_map(5,2)
